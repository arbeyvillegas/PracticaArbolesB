...
//    }
    ...
    /**
     * Finds whether the given element is already in the tree.
     */
    public T findMember(T info) {
    	return findMember(info,root);
    }
    ...
    private T findMember(T info,Node node) {
    	T member=null;
    	if (node == null) {
            member = null;
        } else if (info.compareTo(node.information) == 0) {
            ...
            member = findMember(info, node.right);
        } else {
            member = findMember(info, node.left);
        }
    	return member;
    }
