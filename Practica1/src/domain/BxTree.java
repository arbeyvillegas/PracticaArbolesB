...
public class BxTree<Key extends Comparable<? super Key>, Value> {
	/**
	 * Pointer to the root node. It may be a leaf or an inner node, but it is
	 * never null.
	 */
	private Node root;
	...
	/**
	 * Looks for the given key. If it is not found, it returns null. If it is
	 * found, it returns the associated value.
	 */
	@SuppressWarnings("unchecked")
	public Value find(Key key) {
		Node node = root;
		while (node instanceof BxTree.INode) { // need to traverse down to the
												// leaf
	...
	public class LNode extends Node {
		...
		public Split insert(Key key, Value value) {
			...
			if (this.num == M) { // The node was full. We must split it
				...
				Split result = new Split(sibling.keys[0], // make the right's
															// key >= result.key
						this, sibling);
				return result;
			} else {
		...
		private void insertNonfull(Key key, Value value, int idx) {
			...
			if (idx < num && keys[idx].equals(key)) {
				// We are inserting a duplicate value, simply overwrite the old
				// one
				values[idx] = value;
			} else {
	...
	public class INode extends Node {
		...
		public Split insert(Key key, Value value) {
			/*
			 * Early split if node is full. This is not the canonical algorithm
			 * for B+ trees, but it is simpler and it does break the definition
			 * which might result in immature split, which might not be desired
			 * in database because additional split lead to tree's height
			 * increase by 1, thus the number of disk read so first search to
			 * the leaf, and split from bottom up is the correct approach.
			 */
			...
			if (this.num == N) { // Split
				...
				this.num = mid - 1;// this is important, so the middle one
									// elevate to next depth(height), inner
		...
		private void insertNonfull(Key key, Value value) {
			...
			if (result != null) {
				if (idx == num) {
					...
					System.arraycopy(keys, idx, keys, idx + 1, num - idx);
					System.arraycopy(children, idx, children, idx + 1, num
							- idx + 1);
